...
With unbalanced classes, logistic regression can have trouble as the intercept goes to far negative or positive. However, these classes are not severely unbalanced, so I'm going to start this analysis without any special handling of unbalanced classes.
...
I could imagine also using some kind of log-linear model, but that just feels like a special case of logistic regression without all the nice properties afforded by the Bayesian approach.
